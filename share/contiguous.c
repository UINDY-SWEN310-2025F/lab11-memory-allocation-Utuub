...
void firstFit(int nSeg, int nProc, int segSize[], int segUsed[], int segStart[], AllocationInfo alloc[]) {
    ...
    for(int p =0; p < nProc; p++) {
        int need = alloc[p].size;

        for(int s = 0; s < nSeg; s++) {
            if(!occupied[s] && segSize[s] >= need) {
                // Allocate here
                alloc[p].partition = s;
                alloc[p].start = segStart[s];
                alloc[p].end = segStart[s] + need;
                occupied[s] = 1;
                segUsed[s] = need;
                break;
            }
...
void bestFit(int nSeg, int nProc, int segSize[], int segUsed[], int segStart[], AllocationInfo alloc[]) {
    ...
    for(int p = 0; p < nProc; p++) {
        ...
        if(best != -1) {
            alloc[p].partition = best;
            alloc[p].start = segStart[best];
            alloc[p].end = segStart[best] + need;
            occupied[best] = 1;
            segUsed[best] = need;
        }
    }
